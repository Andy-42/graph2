...
import andy42.graph.model.{NodeId, Event, EventTime}
...
trait StandingQueryEvaluation:
  ...
  /** A node has changed. The implementation will attempt to match all standing queries using this node as a starting
    ...
    *
    * Since far edged might not be synced yet, when we pull in node for matching, we can patch the far edges up to that
    * they are consistent.
    */
...
final case class StandingQueryEvaluationLive(graph: Graph) extends StandingQueryEvaluation:
   ...
   * 
   * Events are only appended to one node at one time, but it may include edge events that have only been applied
   * to the near node and are not yet reflected in the Graph. We expand the set of affected nodes so that these other
   * nodes appear with their far edge events added. This allows matching to be done with a consistent image of the changes.
   * 
   * We may choose to not match against standing queries if the newEvents are purely far edge events.
   * This creates a possiblity of missing a match, so this behaviour is configurable.
   */
  override def nodeChanged(node: Node, time: EventTime, newEvents: Vector[Event]): IO[UnpackFailure | PersistenceFailure, Unit] =
  ...
  private def matchAgainstStandingQueries(changedNodes: Map[NodeId, Node], time: EventTime): IO[UnpackFailure | PersistenceFailure, Unit] = ???
