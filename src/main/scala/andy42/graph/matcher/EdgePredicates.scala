...
extension (edgeSpec: EdgeSpec)
  ...
  def edgeKeyIsOneOf(ss: Set[String]): EdgeSpec = edgeSpec.withKeyPredicate(EdgeKeyIsOneOf(ss))
  def edgeKeyMatches(r: Regex): EdgeSpec = edgeSpec.withKeyPredicate(EdgeKeyMatches(r))
