...
object EventHasEffectOps:
  ...
  /** Remove any events that would have no effect on the given snapshot. This is calculated in a way that does not copy
    * `events` if there is no change to the state. This avoids heap allocation pressure for this frequently executed
    * operation.
    */
  ...
  /** Determine if an Event would have any effect if applied to a Node's history given the collapsed node snapshot
    * calculated for that point in time.
    */
